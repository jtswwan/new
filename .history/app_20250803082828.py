...
def call_gemini_sync(prompt: str, model: str = "gemini-1.5-flash"):
    # 使用 GenAI SDK 呼叫模型，傳入 prompt 和設定
    return client.models.generate_content(
        model=model,
        contents=prompt,
        config={
            "temperature": 0.0,  # 設定生成內容的隨機性
        }
    )

# 定義 API 路由，處理 POST 請求以獲取 AI 建議
@app.route("/api/suggestions", methods=["POST"])
def api_suggestions():
    try:
        # 解析 JSON 請求資料
        data = request.get_json(force=True) or {}
        ac_temp = data.get("ac_temp", 26)  # 空調設定溫度，預設為 26°C
        room_template = data.get("room_template", "custom")  # 房型模板，預設為自訂
...
"""
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
            ...
            try:
                resp = fut.result(timeout=25)  # 等待模型回應
