...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!suggestionBtn || !suggestionBlock || !suggestionList) return;
  ...
  function fillAISuggestions() {
    const tips = [
      ...
      '下午西曬時，空調 26–27°C + 低速循環扇更省電。'
    ];
    suggestionList.innerHTML = tips.map(t => `<li>${t}</li>`).join('');
  }

  // 點按按鈕：更新 → 顯示/收起 → 若顯示就捲動過去
  suggestionBtn.addEventListener('click', () => {
    // 如果你已有別處定義的 getAISuggestions()，可改呼叫那個
    fillAISuggestions();

    // 想「只顯示、不收起」：改成 classList.remove('hidden')
    suggestionBlock.classList.toggle('hidden');
